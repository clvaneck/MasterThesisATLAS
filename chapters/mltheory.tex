...
A common nonlinearity for hidden layers is the rectified linear unit~\cite{Nair:2010relu},
\begin{equation}
...
A standard regulariser used throughout this thesis is \emph{dropout}~\cite{Srivastava:2014dropout}. During training, each hidden unit is set to zero with probability $p$ independently for every example, and the surviving activations are rescaled by $1/(1-p)$ so that the expected output magnitude is preserved. The procedure prevents co-adaptation of hidden units and approximates an exponentially large ensemble of subnetworks. At evaluation time the full network is used with all units active.
...
\section{Distance correlation decorrelation}
\label{sec:disco}

A standard requirement on a classifier used in a physics analysis is that its output be statistically independent of an observable that the downstream analysis needs to keep available, such as a jet mass on which background sidebands are defined. The \emph{distance correlation} (DisCo) of two random variables $X$ and $Y$, $\mathrm{dCor}(X,Y)\in[0,1]$, is zero if and only if $X$ and $Y$ are independent, in contrast to the linear Pearson correlation which only detects monotonic dependence. Adding the empirical distance correlation between the classifier output and the protected observable to the training loss as a penalty,
\begin{equation}
\mathcal{L} = \mathcal{L}_{\text{class}} + \lambda\,\mathrm{dCor}^{2}\!\big(\hat{y},m\big),
\end{equation}
drives the trained classifier towards outputs that are statistically independent of $m$ while still discriminating the signal from the background~\cite{Kasieczka:2020DisCo}. The strength of the decorrelation is controlled by the hyperparameter $\lambda$, which is tuned to balance the classification loss against the residual sculpting of the protected observable.