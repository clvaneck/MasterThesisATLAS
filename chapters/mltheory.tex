...
Machine learning provides a framework for constructing functions that map inputs to outputs by optimising a parametric model against labelled examples.  In high-energy physics the canonical task is classification: given a set of measured quantities describing an event, assign it to one of $K$ classes.  The theoretical tools introduced in this chapter, feedforward networks, recurrent networks, and transformers, address this task with increasing architectural complexity.  Throughout, the mathematical objects that are common to all architectures, activation functions, loss functionals, and gradient-based optimisation, are introduced as they first arise.
...
\end{equation}
Standardisation ensures that features with different physical scales contribute equally to the gradient updates and prevents large-magnitude inputs from dominating the loss landscape.  The statistics $\mu_j$ and $\sigma_j$ are computed on the training set and applied unchanged to validation and test sets to avoid information leakage.
...
\begin{figure}[htbp]
  ...
  \caption{Unrolled LSTM cell showing three consecutive time steps.  The expanded central cell exposes the internal gating structure: the forget gate ($\sigma$), input gate ($\sigma$), candidate cell update ($\tanh$), and output gate ($\sigma$) each receive the concatenation of the previous hidden state $\mathbf{h}_{t-1}$ and the current input $\mathbf{x}_t$.  The cell state $\mathbf{c}_t$ flows along the horizontal additive path (upper horizontal line), enabling long-range gradient propagation; the hidden state $\mathbf{h}_t$ is derived from it via the output gate and a $\tanh$ squashing, as described by equations~\eqref{eq:lstm_cell} and~\eqref{eq:lstm_hidden}.  Diagram adapted from Olah~\cite{Olah_LSTM_2015}.}
  \label{fig:lstm_cell}
\end{figure}
