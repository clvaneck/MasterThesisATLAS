...
\label{eq:softmax}
\end{equation}
where $z_k$ are the pre-activations of the final layer.  By construction $\hat{y}_k\in(0,1)$ and $\sum_k\hat{y}_k=1$, so the output vector forms a valid discrete probability distribution over the $K$ classes.  For $K=2$ the softmax reduces to Equation~\eqref{eq:bce}.  The corresponding loss is the categorical cross-entropy,
\begin{equation}
