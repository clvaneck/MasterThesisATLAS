...
A multi-layer perceptron (MLP) is a feedforward neural network organised into $L$ sequential layers.  Each layer $\ell$ maps an input vector $\mathbf{h}^{(\ell-1)}\in\mathbb{R}^{n_{\ell-1}}$ to an output $\mathbf{h}^{(\ell)}\in\mathbb{R}^{n_{\ell}}$ by a linear transformation followed by a pointwise nonlinearity $\sigma$,
\begin{equation}
...
In the context of aQGC searches, the multi-class formulation is particularly natural.  Each operator family defines a distinct signal class and the softmax output $\hat{y}_k$ provides a per-event estimate of the posterior probability for operator $k$.  A single multi-class network can therefore simultaneously discriminate among operator types and separate them from the background, replacing the combination of multiple binary classifiers that would otherwise be required.
...
\end{align}
where $\sigma$ is the sigmoid activation.  Each gate is a vector in $(0,1)^d$ whose entries act as soft switches.  A potential update to the cell state is computed as
\begin{equation}
