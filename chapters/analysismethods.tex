...
A training selection, designed to bring the phase space close to the analysis signal region without yet defining it exactly, is obtained by requiring a VBF-RNN v3 score above 0.3 and a DisCoJet score above 0.65 for both large-$R$ jets, as summarised in Table~\ref{tab:mlp_selection}, the MLP-based discriminant is trained and evaluated within this selection.
...
The expected sensitivity is quantified using the Asimov binned significance. The Asimov dataset, introduced in Ref.~\cite{Cowan:2010js}, is the single representative dataset in which every observed bin count is set equal to its expectation value under the tested hypothesis; evaluating the profile-likelihood test statistic on this dataset yields the median expected significance without the need for pseudo-experiments. For a counting experiment with signal yield $s_i$ and background yield $b_i$ in bin $i$, the resulting median discovery significance reduces to
\begin{equation}
Z_A = \sqrt{2\sum_i\left[(s_i + b_i)\ln\left(1 + \frac{s_i}{b_i}\right) - s_i\right]},
\end{equation}
where $s_i$ and $b_i$ are the signal and background yields in bin $i$. The more familiar approximation $Z \approx s/\sqrt{b}$ is recovered from $Z_A$ in the limit $s_i \ll b_i$, but breaks down once the signal yield becomes comparable to or exceeds the background. In several of the high-score bins of the multivariate discriminants studied here, the signal yield does exceed the background yield, so the Gaussian $s/\sqrt{b}$ approximation would significantly overestimate the sensitivity. The full Asimov expression remains valid in this regime, as it is derived from the Poisson likelihood without assuming $s \ll b$, and is therefore used throughout this work. To enable a direct comparison between operators with widely different production cross sections and between the various classifier architectures, results are presented normalised to the significance $Z_A^{m_{JJ}}$ obtained from a one-dimensional fit of the dijet invariant mass $m_{JJ}$ alone, computed with the same Asimov procedure and the same minimum-background-yield binning prescription. The dijet mass is the most discriminating single high-level variable in VBS-like topologies, and is used as the baseline observable in the existing ATLAS VBS measurements that this analysis builds upon. Quoting the ratio $Z_A / Z_A^{m_{JJ}}$ therefore expresses the gain in sensitivity that the multivariate discriminants provide over the conventional cut-and-count style baseline, while cancelling overall normalisation effects (luminosity, signal cross section) that would otherwise dominate absolute significance values and obscure the comparison between models.