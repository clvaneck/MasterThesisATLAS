...
The physical process targeted by this analysis is the electroweak production of two vector bosons in association with two jets, $pp \to VVjj$~(EWK). This inclusive EWK process is produced as a single sample that contains several distinct contributions to the same $VVjj$ final state, illustrated in Figure~\ref{fig:ewk_diagrams}. The genuine VBS subprocess (Figure~\ref{fig:vbs_ewk}) proceeds through a $VV \to VV$ scattering vertex and is the signal of interest. It is accompanied by radiative diagrams in which a vector boson is emitted directly from a quark line (Figure~\ref{fig:novbs_ewk}); the split between the genuine scattering diagrams and these radiative diagrams is not gauge invariant, so the radiative contribution cannot be isolated unambiguously and is retained together with the VBS signal. The remaining contributions arise from genuinely distinct hard processes that \emph{are} identifiable at truth level: triboson production, $pp \to VVV$ with one boson decaying hadronically (Figure~\ref{fig:vvv_ewk}), and top-associated production such as $tZq$ (Figure~\ref{fig:tzq_ewk}). These two define the non-VBS EWK background. The objective of the VBF-RNN tagger is therefore not to separate VBS from every other EWK $VVjj$ diagram, but to enrich the sample in the VBS topology by rejecting the separable triboson and top-associated contributions.
...
\label{fig:tzq_ewk}
\end{subfigure}
\caption{Representative diagrams contributing to the inclusive electroweak $pp \to VVjj$~(EWK) sample. (a) The VBS subprocess, in which the two incoming quarks each radiate a vector boson that subsequently scatter, $VV \to VV$, producing the two forward tagging quarks and the two final-state bosons. (b) A radiative non-scattering EWK diagram, in which a vector boson is emitted from a quark line, this contribution is not separable in a gauge invariant way from the VBS amplitude and is therefore kept together with the VBS signal. (c) A triboson ($VVV$) diagram and (d) a top-associated ($tZq$) diagram, which reach the same $VVjj$ final state through distinct hard processes that are identifiable at truth level and together form the non-VBS EWK background the tagger is trained to suppress.}
