...
Events are selected through a sequential pipeline. First, a pre-selection is applied requiring at least two large-$R$ jets, a leading signal jet $p_T > 560$~GeV, a sub-leading signal jet $p_T > 200$~GeV and a dijet invariant mass $m_{JJ} > 1.3$~TeV. The pre-selection criteria are summarised in Table~\ref{tab:analysis_selection}.
...
The physical process targeted by this analysis is the electroweak production of two vector bosons in association with two jets, $pp \to VVjj$~(EWK), with both bosons decaying to large-$R$ jets. This inclusive EWK process is produced as a single sample that contains several distinct contributions to the same $VVjj$ final state, illustrated in Figure~\ref{fig:ewk_diagrams}. The genuine VBS subprocess (Figure~\ref{fig:vbs_ewk}) proceeds through $VV \to VV$ scattering and is the signal of interest. It is accompanied by radiative diagrams in which a vector boson is emitted directly from a quark line (Figure~\ref{fig:novbs_ewk}). The split between the genuine scattering diagrams and these radiative diagrams is not gauge invariant, so the radiative contribution cannot be isolated and is retained together with the VBS signal. The remaining contributions arise from genuinely distinct hard processes that are identifiable at truth level: triboson production ($pp \to VVV$ with one boson decaying to two small-$R$ jets, Figure~\ref{fig:vvv_ewk}) and top-associated production such as $tZq$ (Figure~\ref{fig:tzq_ewk}). These two define the non-VBS EWK background.

\begin{figure}[htbp]
\centering
\begin{subfigure}[b]{0.43\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/vbs_ewk.pdf}
    \caption{}
    \label{fig:vbs_ewk}
\end{subfigure}
\quad
\begin{subfigure}[b]{0.43\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/novbs_ewk.pdf}
    \caption{}
    \label{fig:novbs_ewk}
\end{subfigure}
...
\begin{subfigure}[b]{0.43\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/vvv_ewk.pdf}
    \caption{}
    \label{fig:vvv_ewk}
\end{subfigure}
\quad
\begin{subfigure}[b]{0.43\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/tzq_ewk.pdf}
    \caption{}
    \label{fig:tzq_ewk}
\end{subfigure}
\caption{Representative diagrams contributing to the inclusive electroweak $pp \to VVjj$~(EWK) process. (a) The VBS subprocess, in which the two incoming quarks each radiate a vector boson and emerge as the two forward tagging quarks, while the two radiated bosons scatter, $VV \to VV$, into the two final-state bosons. (b) A radiative non-scattering EWK diagram, in which a vector boson is emitted from each quark line, a contribution that is not separable in a gauge-invariant way from the VBS amplitude and is therefore kept together with the VBS signal. (c) A triboson ($VVV$) diagram and (d) a top-associated ($tZq$) diagram, which reach the same $VVjj$ final state through distinct hard processes that are identifiable at truth level and together form the non-VBS EWK background the tagger is trained to suppress.}
\label{fig:ewk_diagrams}
\end{figure}

The VBF-RNN tagger was originally developed to identify the characteristic forward-jet topology of vector boson fusion and vector boson scattering processes and has been deployed in previous ATLAS measurements including the semi-leptonic VBS analysis~\cite{ATLAS:semileptonicVBS:2025}. In this analysis the tagger is adapted and re-optimised for the fully hadronic VBS channel, where both the tagging jets and the signal jets from the hadronically decaying bosons are available as inputs. The tagging-jet inputs are potential tagging jets rather than truth-identified ones: the up to four highest-$p_T$ small-$R$ PFlow jets of the event, ordered by decreasing transverse momentum so that jet~1 is the highest-$p_T$ jet, jet~2 the next-highest and so on. It is left to the network to learn which of these candidates carry the forward VBS tagging-jet topology.

The classification task is formulated as a supervised binary problem: each event is assigned a label $y = 1$ if it belongs to the VBS signal and $y = 0$ if it belongs to the non-VBS EWK background. The labels are defined at truth level from the hard-scatter content of the event: an event containing two vector bosons and no top quarks is labelled VBS signal, while the remaining events, with at least one top quark ($tX$) or a number of vector bosons not equal to two ($VVV$), are labelled non-VBS EWK background. By construction the signal class therefore also absorbs the inseparable radiative EWK diagrams of Figure~\ref{fig:novbs_ewk}.

Three versions of the tagger are developed for this analysis. They differ in the set of jets provided as input and the training sample used, and are summarised in Table~\ref{tab:vbfrnn_versions}.
...
The four tagging jets constrain only the production side of the event, but the VBS and non-VBS EWK subprocesses also differ in the diboson system they produce. The genuine $VV\to VV$ scattering of the VBS subprocess yields a harder, higher-mass central diboson pair than the non-VBS EWK topologies that share the same final state. Figure~\ref{fig:vbfrnn_sigjet} exposes this residual difference through the dijet invariant mass of the two large-$R$ signal jets, which retains discriminating power between the two classes that the tagging jets do not capture and which is largely statistically independent of the production-side information. The residual difference motivates the six-jet configuration of version~3.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{figures/vbfrnn_sigjet_mjj.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figures/VBFRNN.png}
...
Both the binary and multi-class MLPs are implemented in Keras~\cite{Chollet_Keras_2015} and share a feed-forward architecture of fully connected hidden layers with ReLU activations, followed by an output layer whose form is dictated by the classification task: a single sigmoid node for the binary classifier and four softmax nodes for the multi-class classifier. The multi-class classifier is wider and deeper compared to the binary classifier. The complete set of hyperparameters is given in Table~\ref{tab:mlp_hyperparams}.
