...
\begin{table}[htbp]
  ...
  \begin{tabular}{|c|c|}
    ...
    \textbf{Stage} & \textbf{Requirements} \\
    \hline
    BTS65 SR-like selection & VBF-RNN v3 score $> 0.3$ \\
...
\end{subfigure}
\caption{Normalised distributions of the jet multiplicity and VBF-RNN input features in the BTS65 SR-like selection. Top row: total number of small-$R$ jets and number of large-$R$ jets; bottom row: number of RNN tagging jets and VBF-RNN score. Distributions are shown for representative S0 (red), M0 (green) and T0 (orange) aQGC signal samples and the composite background (blue). Signal events tend to have higher jet multiplicities and are strongly peaked at high VBF-RNN scores, reflecting their characteristic VBS topology.}
